    ...
    */
    $('.left-nav #nav li').click(function(event) {
        if($(this).children('.sub-menu').length) {
            ...
            var url = $(this).children('a').attr('id');
    ...
    */
    function CustomRightClick(id) {
        ...
        $('.layui-tab-title li').on('contextmenu', function(e) {
            ...
            //alert(popupmenu.find("li").attr("data-id")+';当前X：'+e.clientX+';当前y：'+e.clientY+';当前系统长度：'
            //+$(document).width()+';当前系统高度：'+$(document).height()+';实际i'+l+';实际t:'+t);
            popupmenu.css({
                left: l,
                top: t
            }).show();
    ...
    */
    ...
    window.WeAdminShowBtn = function(title, url, w, h) {
        ...
        layer.open({
            ...
            skin:'layer-ext-myskin',
            btn: [
                '<i class="layui-icon">&#xe605;'+ '确定'+'</i>',
                '<i class="layui-icon">&#x1006;'+ '取消'+'</i>'],
            btnAlign: 'c',
...
/*第二中带确定按钮的*/
/*弹出层+传递ID参数*/
window.WeAdminEditBtn = function(title, url, id, w, h , top) {
    if(title == null || title == '') {
        title = false;
    };
    if(url == null || url == '') {
    ...
    layer.open({
        ...
        skin:'layer-ext-myskin',
        btn: [
            '<i class="layui-icon">&#xe605;'+ '确定'+'</i>', '<i class="layui-icon">&#x1006;'+ '取消'+'</i>'
        ],
        btnAlign: 'c',
        yes: function(index, layero){
            var body = layer.getChildFrame('body', index); //得到iframe页面层的BODY
    ...
    */
    ...
    function setStorageCurMenu() {
        ...
        if(curMenu.title == '我的桌面') $("#homeContent").attr("src" , curMenu.url)
